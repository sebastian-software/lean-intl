...
import { toLatinUpperCase } from "./6.locales-currencies-tz.js"
...
import { FormatNumber } from "./11.numberformat.js"

import { generateSyntheticFormat, createDateTimeFormats } from "./cldr"
 ...
 */
export function /* 12.1.1.1 */ InitializeDateTimeFormat(
  dateTimeFormat,
  locales,
  options
) {
  ...
  for (let prop in dateTimeComponents) {
    ...
    if (hop.call(bestFormat, prop)) {
      ...
      {
        // diverging
        p = bestFormat._ && hop.call(bestFormat._, prop) ?
          bestFormat._[prop] :
          p
      }
...
function BasicFormatMatcher(options, formats) {
  ...
  while (i < len) {
    ...
    for (let property in dateTimeComponents) {
      ...
      let formatProp = hop.call(format, property) ?
        format[property] :
        undefined
...
function BestFitFormatMatcher(options, formats) {
     ...
     */
  {
    ...
    if (optionsPropNames.length === 1) {
      const bestFormat = generateSyntheticFormat(
        optionsPropNames[0],
        options[`[[${optionsPropNames[0]}]]`]
      )
      if (bestFormat) {
        return bestFormat
      }
  ...
  while (i < len) {
    ...
    for (let property in dateTimeComponents) {
      ...
      let patternProp = hop.call(format._, property) ?
        format._[property] :
        undefined
      ...
      if (optionsProp === undefined && formatProp !== undefined)
...

        {
          ...
          if (
            formatPropIndex <= 1 && optionsPropIndex >= 2 ||
            formatPropIndex >= 2 && optionsPropIndex <= 1
          ) {
            // 5. If delta = 2, decrease score by longMorePenalty.
...
function CreateDateTimeParts(dateTimeFormat, x) {
  ...
  // 11.1.3.
  let nf2 = new Intl.NumberFormat([ locale ], {
  ...
  let localeData = internals.DateTimeFormat["[[localeData]]"][
    dataLocale
  ].calendars
