...
function computeFinalPatterns(formatObj) {
  ...
  formatObj.pattern12 = formatObj.extendedPattern.replace(
    /'([^']*)'/g,
    ($0, literal) => literal ? literal : "'"
  )

  // pattern 12 is always the default. we can produce the 24 by removing {ampm}
  formatObj.pattern = formatObj.pattern12
    .replace("{ampm}", "")
...
function expDTComponentsMeta($0, formatObj) {
  switch ($0.charAt(0)) {
    ...
    // --- Era
    case "G":
      formatObj.era = [ "short", "short", "short", "long", "narrow" ][
        $0.length - 1
      ]
      return "{era}"
    ...
    case "Q":
    case "q":
      formatObj.quarter = [ "numeric", "2-digit", "short", "long", "narrow" ][
        $0.length - 1
      ]
      return "{quarter}"
    ...
    case "M":
    case "L":
      formatObj.month = [ "numeric", "2-digit", "short", "long", "narrow" ][
        $0.length - 1
      ]
      return "{month}"
...
export function createDateTimeFormat(skeleton, pattern) {
  ...
  formatObj.extendedPattern = pattern.replace(
    expDTComponents,
    ($0) => // See which symbol we're dealing with
    expDTComponentsMeta($0, formatObj._)
  )
  ...
  skeleton.replace(expDTComponents, ($0) => // See which symbol we're dealing with
  expDTComponentsMeta($0, formatObj))
...
export function createDateTimeFormats(formats) {
  ...
  for (i = 0; i < timeRelatedFormats.length; i += 1) {
    for (j = 0; j < dateRelatedFormats.length; j += 1) {
      ...
      computed = joinDateAndTimeFormats(
        dateRelatedFormats[j],
        timeRelatedFormats[i]
      )
...
export function generateSyntheticFormat(propName, propValue) {
  if (
    validSyntheticProps[propName] && validSyntheticProps[propName][propValue]
  ) {
    return {
