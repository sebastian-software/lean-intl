...
export const arrIndexOf = Array.prototype.indexOf ||
  function(search) {
...
export const objCreate = Object.create ||
...
export const fnBind = Function.prototype.bind ||
  function(thisObj) {
    let fn = this,
...
export function createRegExpRestore() {
  ...
  return function() {
    ...
    if (has) {
      for (let i = 1; i <= 9; i++) {
        ...
        if (!m) {
          ...
          lastMatch = `${m})${lastMatch.substring(
            lastMatch.indexOf(m) + m.length
          )}`
        }
      }
    }
    ...
    exprStr = exprStr.replace(
      /((^|[^\\])((\\\\)*\\[()])+|[^()])+/g,
      (match) => `[\\s\\S]{${match.replace(/\\(.)/g, "$1").length}}`
    )
