var IntlPolyfill = (function () {
  ...
  const es3 = !realDefineProp && !Object.prototype.__defineGetter__;

  // We use this a lot (and need it for proto-less objects)
  const hop = Object.prototype.hasOwnProperty;

  // Naive defineProperty for compatibility
  const defineProperty = realDefineProp ? Object.defineProperty : function (obj, name, desc) {
  ...
  const arrIndexOf = Array.prototype.indexOf || function (search) {
  ...
  const objCreate = Object.create || function (proto, props) {
      let obj;
  ...
  const arrJoin   = Array.prototype.join;
  const arrShift  = Array.prototype.shift;

  // Naive Function.prototype.bind for compatibility
  const fnBind = Function.prototype.bind || function (thisObj) {
      let fn = this,
  ...
  const internals = objCreate(null);
  ...
  */
  ...
  let expBCP47Syntax = RegExp('^(?:'+langtag+'|'+privateuse+'|'+grandfathered+')$', 'i');

  // Match duplicate variants in a language tag
  let expVariantDupes = RegExp('^(?!x).*?-('+variant+')-(?:\\w{4,8}-(?!x-))*\\1\\b', 'i');

  // Match duplicate singletons in a language tag (except in private use)
  let expSingletonDupes = RegExp('^(?!x).*?-('+singleton+')-(?:\\w+-(?!x-))*\\1\\b', 'i');

  // Match all extension sequences
  let expExtSequences = RegExp('-'+extension, 'ig');
  ...
  function formatToParts$1(date = undefined) {
      let internal = this !== null && typeof this === 'object' && getInternalProperties(this);
